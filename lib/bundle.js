  ...
  _createClass(MovingObject, [{
    ...
    value: function collideIntoPlayer(ship) {
      ...
      if (x < 70 && y < 40) {
        ...
        this.game.stage.removeChild(this.bullet);
        if (ship.health <= 0) {
          (0, _explosion2.default)(ship.ship.x, ship.ship.y, this.game.stage);
          this.game.audio['explosion'].play();
  ...
  _createClass(Game, [{
    ...
    value: function initializeTicker() {
      ...
      this.ticker.addEventListener('tick', function () {
        _this3.ship.shipEvents();
        _this3.moveAllBullets();
        _this3.fireEnemyBullets();
        _this3.moveAllEnemyBullets();
        _this3.moveAllEnemies();
        _this3.stage.update();
      });
    }
  }, {
...
var Enemy = exports.Enemy = function (_MovingObject) {
  ...
  _createClass(Enemy, [{
    key: 'movedown',
    value: function movedown(ticks, y) {
      if (this.tweening === false) {
        this.shooting = true;
        this.tweening = 'charging';
        createjs.Tween.get(this.enemy, { useTicks: true, override: true }).wait(5).to({ y: y }, 60, createjs.Ease.backOut);
      }
    }
  }]);
