  ...
  _createClass(MovingObject, [{
    ...
    value: function dropPowerUp(enemy) {
      ...
      if (random < .30 && this.game.powerupCooldown === false) {
        ...
        setTimeout(function () {
          _this.game.powerupCooldown = false;
        }, 8000);
      }
    }
  }, {
  ...
  _createClass(Game, [{
    ...
    value: function toggleSound() {
      ...
      if (this.muted === false) {
        ...
        Object.keys(this.audio).forEach(function (audio) {
          _this2.audio[audio].muted = true;
        });
      } else {
        this.muted = true;
        $('#audio-button').css('color', 'white');
        Object.keys(this.audio).forEach(function (audio) {
          _this2.audio[audio].muted = false;
        });
      }
    }
  }, {
    ...
    value: function initializeTicker() {
      ...
      this.ticker.addEventListener('tick', function () {
        if (!_this3.paused && _this3.gameStarted) {
          _this3.tick();
        }
      });
    }
  ...
  _createClass(Enemy, [{
    ...
    value: function spiralAttack() {
      ...
      var id = setInterval(function () {
        ...
        var x = 3 * Math.cos(r);
        var y = 3 * Math.sin(r);
        var bullet = new _bullet.TieFighterBullet(x, y, _this3.game);
        bullet.addBullet(_this3.enemy.x, _this3.enemy.y);
        _this3.game.enemyBullets.push(bullet);

        i += 1;

        if (i >= 80 || _this3.health <= 0) {
          clearInterval(id);
        }
      }, 80);
    }
