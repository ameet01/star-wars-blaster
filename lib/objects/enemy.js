...
export class Enemy extends MovingObject {
    ...
    if(this.tweening === false) {
      ...
      .call(() => {
        ...
        this.game.bullets.forEach(bullet => {
          if(Math.abs(bullet.bullet.y - this.enemy.y) < 200) {
            bullet.active = false;
          }
        });
        this.shooting = true;
  ...
  spiralAttack() {
    ...
    var id = setInterval(() => {
      ...
      var bullet = new SpiralBullet(x,y,this.game);
      ...
      if (i >= 82 || this.game.ship.health <= 0 || this.health <= 0) {
        clearInterval(id);
      }
    }, 51);
  }
...
export class DeathStar extends Enemy {
  constructor(velocityX, velocityY, game, typeofmove) {
    ...
    this.enemy.scaleX = window.innerWidth/1000 * 0.08;
    this.enemy.scaleY = window.innerWidth/1000 * 0.08;
    this.tweening = false;
    this.shooting = false;
    this.typeofmove = typeofmove;
  ...
  movedown(y) {
    if(this.tweening === false) {
      ...
      .call(() => {
        if(this.game.gameStarted && this.game.boss) {
          this.spiralAttack();
        }
      })
      .wait(50).to({x: this.enemy.x - 300}, 60)
